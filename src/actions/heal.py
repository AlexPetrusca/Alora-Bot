...
class HealAction(Action):
    ...
    def tick(self, tick_counter):
        ...
        if tick_counter == Timer.sec2tick(13):
            robot.click_image(Images.HEAL_OPTION, 0.9)
        if self.bank:
            if tick_counter == Timer.sec2tick(16):
                robot.click(HealActionCoord.BANK_CHEST)  # click bank chest
            if tick_counter == Timer.sec2tick(20):
                robot.click(BankMenu.CLOSE)  # close bank
            if tick_counter == Timer.sec2tick(21):
                return Action.Status.COMPLETE
        else:
            if tick_counter == Timer.sec2tick(14):
                return Action.Status.COMPLETE
